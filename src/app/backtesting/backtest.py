...
FIXES:
  - Added CrtDetector import
  - Added _find_entry() method to route by entry_model (candle_pattern, crt, all)
  - Replaced hardcoded RejectionDetector with _find_entry() call
  - Now matches signal_service.py behavior for entry detection
"""
...
class BacktestReport:
    ...
    def print(self) -> None:
        ...
        pairs = sorted(
            set((x.signal.htf_interval, x.signal.ltf_interval) for x in self.results)
        )
        if len(pairs) > 1:
    ...
    def _print_summary(
        ...
        if not compact:
            ...
            print(
                f"  Signals={len(r)}  W={len(wins)} BE={len(bes)} L={len(losses)}  Inv/Exp={len(invals)}/{len(expd)}"
            )

        print(
            f"  {'Win rate (closed)':<32} {'%s%.1f%%%s' % (GREEN if win_rate >= 50 else RED, win_rate, R)}"
        )
        print(
            f"  {'Profit factor':<32} {'%s%.2f%s' % (GREEN if pf >= 1 else RED, pf, R)}"
        )
        print(
            f"  {'Total R':<32} {'%s%+.2fR%s' % (GREEN if total_r >= 0 else RED, total_r, R)}"
        )
        if wins or bes:
        ...
        for dir_label, subset in [("LONG", longs), ("SHORT", shorts)]:
            ...
            print(
                f"  {dir_label:<8}  W={len(w)} BE={len(b)} L={len(l)}  WR={wr:.0f}%  {col}{tr:+.2f}R{R}"
            )
        print(f"{W}{sep}{R}")
...
class MultiPairBacktester:
    ...
    def __init__(
        ...
        # Validate entry_model
        assert self.cfg.entry_model in ("candle_pattern", "crt", "all"), (
            f"Invalid entry_model: {self.cfg.entry_model}. "
            "Must be 'candle_pattern', 'crt', or 'all'."
        )
        logger.info("[%s] Entry model: %s", symbol, self.cfg.entry_model)
    ...
    def _find_entry(
        ...
        """
        Route to the correct entry detector based on cfg.entry_model.

        Mirrors signal_service._find_entry() for consistency.

        CANDLE_PATTERN: candles_entering_ltf() → RejectionDetector
        CRT:            ltf_zone directly → CrtDetector
        ALL:            both, max by timestamp
    ...
    def run(self) -> BacktestReport:
        ...
        for ltf_i in range(ltf_lb, n):
            ...
            for htf_interval, ltf_interval in self.pairs:
                ...
                htf_w = [c for c in htf_all if c.timestamp <= current_ts][
                    -htf_lookback:
                ]
                ...
                if profile.use_trend_filter:
                    structure = MarketStructure.detect(
                        htf_w, pivot_bars=self.cfg.pivot_bars
                    )
                ...
                for htf_range in SwingDetector.find_htf_ranges(
                    ...
                    # FIX: Use _find_entry() to route by entry_model instead of hardcoded RejectionDetector
    ...
    def _simulate(self, signal: TradeSignal, future: list[Candle]) -> BacktestResult:
        ...
        for c in future:
            ...
            sl_hit = (
                c.high >= signal.stop_loss if is_short else c.low <= signal.stop_loss
            )
            if sl_hit:
                outcome = (
                    SignalOutcome.BREAKEVEN
                ...
                close_px = (
                    signal.entry_price if (tp1_hit and use_be) else signal.stop_loss
                )
                close_ts = c.timestamp
                break
...
def main() -> None:
    ...
    if args.symbol:
        ...
        for htf_tf in unique_htf:
            if range_start_ts is not None:
                candles = load_from_api_range(
                    symbol, htf_tf, range_start_ts, range_end_ts, cfg
                )
            else:
            ...
            print(
                f"  HTF {htf_tf}: {len(candles)} bars  {cfg.dt_ms(candles[0].timestamp)} → {cfg.dt_ms(candles[-1].timestamp)}"
            )
        ...
        for ltf_tf in unique_ltf:
            ...
            print(
                f"  LTF {ltf_tf}: {len(candles)} bars  {cfg.dt_ms(candles[0].timestamp)} → {cfg.dt_ms(candles[-1].timestamp)}"
            )
    else:
        if not args.csv_ltf:
