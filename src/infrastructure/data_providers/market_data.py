...
class MarketDataClient:
    ...
    def _post(self, path: str, body: dict) -> dict:
        ...
        for attempt in range(1, self._RETRY_ATTEMPTS + 1):
            ...
            try:
                ...
                if self._metrics_fn:
                    self._metrics_fn(symbol, interval, "local", called_at, duration_ms, True, None)
                return resp.json()
            except requests.RequestException as exc:
                last_exc    = exc
                duration_ms = (time.perf_counter() - t0) * 1000
                if self._metrics_fn:
                    self._metrics_fn(symbol, interval, "local", called_at, duration_ms, False, str(exc))
                if attempt < self._RETRY_ATTEMPTS:
                    wait = self._RETRY_BACKOFF[attempt - 1]
                    logger.warning(
                        "[%s %s] MT5 fetch failed (attempt %d/%d), retrying in %.0fs: %s",
                        symbol, interval, attempt, self._RETRY_ATTEMPTS, wait, exc,
                    )
                    time.sleep(wait)
    ...
    def fetch_candles(
        ...
        response = self._post("/time-series", {
            "symbols":    [symbol],
        ...
        if isinstance(raw, dict) and "error" in raw:
            raise MarketDataError(
                f"MT5 server error for {symbol}/{mt5_interval}: {raw['error']}"
            )
        candles = sorted(_parse_candles(raw), key=lambda c: c.timestamp)
    ...
    def fetch_candles_range(
        ...
        logger.info(
            ...
            symbol, interval,
            _ms_to_utc_str(start_ts), _ms_to_utc_str(end_ts),
        )
        ...
        while cursor <= end_ts:
            ...
            batch = sorted(_parse_candles(raw), key=lambda c: c.timestamp)
            if not batch:
                break
        ...
        logger.info(
            ...
            symbol, interval, len(all_candles), batch_num,
        )
        return all_candles
